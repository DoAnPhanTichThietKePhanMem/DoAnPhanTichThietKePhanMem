...
namespace GUI.Views
{
    ...
    {
        private UserServices authenticationDAO = new UserServices();
        public class User
        {
        ...
        private async void btnGui_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (email.Length > 0 && username.Length > 0)
            {
                if (Validation.GetHasError(txtEmail) == false && Validation.GetHasError(txtUserName) == false)
                {
                    try
                    {
                        bool Result = await authenticationDAO.CheckUserForGetPass(email, username);
            ...
            if (password.Length > 0 && confirmpass.Length > 0)
            {
                if (Validation.GetHasError(pwbPassword) == false && Validation.GetHasError(pwbConfirmPass) == false)
                {
                    if (password == confirmpass)
                    {
                        try
                        {
                            bool Result = await authenticationDAO.ResetPass(email, username, password);
                            if(Result == false)
                            {
